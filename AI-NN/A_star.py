# Problem Statement for A * : ABC has to reach to Arad from Bcharest. As there are multiple paths to 
# reach Bucharest help ABC to reach the destination using the shortest path by applying A * Algorithm
...
class PQueue():
    ...
    def _sort(self):
        ...
        # Here key means on on what sorting is applied
        # sorts based on the value in the dictionary
        self.sorted = True
    ...
    with open(path, 'r') as file:
        for line in file:
            ...
            if e2 not in c:
                c[e2] = {}
            c[e1][e2] = c[e2][e1] = v
            #Nested Dictionary d[outer-key][inner-key]: d{"e1":{"e2":v}}
            print(c)
    return c
